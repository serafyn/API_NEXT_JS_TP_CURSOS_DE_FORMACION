...
export default async function loginHandler(req, res) {
    ...
    const reCAPTCHASecretKey = process.env.RECAPCHAT_SECRET_KEY;
    ...
    try {
        ...
        const reCAPTCHAResponse = await axios.post(`https://www.google.com/recaptcha/api/siteverify?secret=${reCAPTCHASecretKey}&response=${captchaToken}`);
        if (!reCAPTCHAResponse.data.success) {
            return res.status(400).json({ error: "Error de validación reCAPTCHA" });
        }
